...
import { Check, Award, Loader2, ArrowRight, Zap, ShieldCheck } from "lucide-react";
...
export function PricingTable({
  ...
  return (
    <div className="space-y-12 font-lazzer">
      {/* Interval Toggle */}
      ...
      <div className={getGridClasses(plans.length)}>
        {plans.map((plan) => {
          ...
          const positioningText =
            plan.positioning ||
            (keyUpper.includes("FREE")
              ? "Find what’s broken"
              : keyUpper.includes("STARTER")
              ? "Keep one site healthy"
              : keyUpper.includes("PRO") || keyUpper.includes("PREMIUM")
              ? "Protect a growing website"
              : keyUpper.includes("AGENCY") || keyUpper.includes("SCALE")
              ? "Monitor multiple client websites"
              : plan.description);

          const capacityLabel =
            plan.monitoredCapacity ||
            ((plan.pageAuditLimit ?? 0) >= 3000
              ? "Up to 3,000 monitored pages"
              : (plan.pageAuditLimit ?? 0) >= 1000
              ? "Up to 1,000 monitored pages"
              : (plan.pageAuditLimit ?? 0) >= 100
          ...
          return (
            <div
              key={plan.id}
              className={cn(
                "relative flex flex-col justify-between rounded-3xl border bg-white p-7 sm:p-8 transition-all hover:shadow-xl",
                plan.isPopular
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-black text-slate-950 font-display">{plan.name}</h3>
                </div>

                {/* Outcome-focused Positioning Subtitle */}
                <p className="mt-1 text-sm font-bold text-emerald-800">
                  {positioningText}
                </p>

                <div className="mt-5 flex items-baseline gap-1.5 border-b border-slate-100 pb-5">
                ...
                {/* Monitored Capacity Pill */}
                <div className="mt-5 rounded-xl bg-[#dff2ed]/50 border border-[#2f7a68]/20 p-2.5 text-xs font-bold text-[#143a31] flex items-center gap-2">
              ...
              <div className="mt-8 pt-6 border-t border-slate-100">
                <button
                  ...
                  className={cn(
                    "w-full rounded-xl py-3 text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm group",
                    plan.isPopular
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <span>{plan.customCtaText || (isFree ? "Audit My Site Free" : `Choose ${plan.name}`)}</span>
