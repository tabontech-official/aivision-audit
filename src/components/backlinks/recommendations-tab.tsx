  ...
  const filteredRecs = useMemo(() => {
    return recommendations.filter((r) => {
      ...
      if (selectedSeverity === "OPPORTUNITY_GOOD") {
        return s === "OPPORTUNITY" || s === "GOOD" || s === "LOW";
      }
      return true;
    });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
        <div className="flex items-center gap-3.5">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-orange-50 text-[#FF4D00] border border-orange-200 shrink-0 font-bold">
            <Sparkles className="h-6 w-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-display text-base font-bold text-slate-900">
                Backlink Optimization Recommendations
              </h2>
              <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-bold text-slate-600 font-sans border border-slate-200">
                {recommendations.length} Action Items
              </span>
            </div>
            <p className="text-xs text-slate-500 font-sans mt-0.5">
              Deterministic rule-based analysis evaluated directly from your real stored backlink profile for{" "}
              <strong className="text-slate-800 font-medium">{domain}</strong>.
            </p>
          </div>
        </div>
      </div>

      {/* 2. Priority Summary Metric Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl border border-blue-200/80 bg-blue-50/30 p-4 shadow-2xs">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-blue-700 font-sans">Profile optimization</span>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex flex-wrap items-center gap-2">
          {[
            { id: "ALL" as const, label: `All (${summary.total})` },
            { id: "CRITICAL_HIGH" as const, label: `Action Required (${summary.criticalHigh})` },
            { id: "MEDIUM" as const, label: `Medium (${summary.medium})` },
            { id: "OPPORTUNITY_GOOD" as const, label: `Opportunities & Healthy (${summary.opportunityGood})` },
          ].map((pill) => {
